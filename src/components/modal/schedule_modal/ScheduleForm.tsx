...
import type {
  Schedule,
  ScheduleFormData,
} from "../schedule_modal/types/schedule";
...
export const ScheduleForm = ({
  ...
  useEffect(() => {
    if (schedule) {
      ...
      setFormData({
        ...
        date: dateStr,
        start_time: start.toISOString().substring(11, 16), // "HH:mm"
        end_time: end.toISOString().substring(11, 16), // "HH:mm"
  ...
  const validateForm = (): boolean => {
    // ScheduleFormData의 필드마다 오류 메시지를 선택적으로 저장하기 위해
    ...
    <form css={formContainer} onSubmit={handleSubmit}>
      ...
      <div css={formRow}>
        ...
        <div css={formGroup}>
          ...
          />
          {errors.start_time && (
            <div css={errorMessage}>{errors.start_time}</div>
          )}
        </div>
      ...
      <div css={formGroup}>
        ...
        />
        {errors.description && (
          <div css={errorMessage}>{errors.description}</div>
        )}
      </div>
      ...
      <div css={formActions}>
        <button css={primaryButton} type="submit" disabled={loading}>
          <MdCheck />{" "}
          {loading ? "저장중..." : schedule ? "수정 완료" : "일정 생성"}
        </button>
        {schedule && onDelete && (
          <button
            css={secondaryButton}
